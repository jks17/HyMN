...
from graphgps.transform.curvature import forman_curvature
...
def compute_posenc_stats(data, pe_types, is_undirected, cfg):
    ...
    for t in pe_types:
        if t not in ['LapPE', 'EquivStableLapPE', 'SignNet', 'RWSE', 'HKdiagSE',
                     'HKfullPE', 'ElstaticSE', 'GraphormerBias', 'DegreeCentrality']:
    ...
    if 'RWSE' in pe_types:
        ...
        distances = torch.sum(rw_landing, dim = 1)
        data.community = torch.argsort(distances, descending=True)
        
    if 'DegreeCentrality' in pe_types:
        G = to_networkx(data, to_undirected=True)

        #curvature = forman_curvature(G)
                #curvature = ollivier_ricci_curvature(G)
        #values = torch.tensor(-curvature.sum(0))
                # normalise values
        #data.pestat_DegreeCentrality= ((values - torch.mean(values))/torch.std(values)).unsqueeze(dim=1).float()
        #data.community = torch.tensor(np.argsort(-curvature.sum(0)))
        #sg = nx.degree_centrality(G)
        sg = nx.subgraph_centrality(G)
        # TODO: consider using a different naming convention than `community` for this
        data.community = torch.tensor(sorted(sg, key=sg.get, reverse=True))
        data.pestat_DegreeCentrality = torch.tensor(list(sg.values())).unsqueeze(dim=1)
        #b=0.5
        #dc = torch.round(pestat_DegreeCentrality/b)*b
        #data.pestat_DegreeCentrality = dc
        #m = torch.mean(pestat_DegreeCentrality)
        #data.pestat_DegreeCentrality = pestat_DegreeCentrality #+ 0.01*torch.randn(len(pestat_DegreeCentrality))
        #data.pestat_DegreeCentrality = (torch.ones(len(pestat_DegreeCentrality))*m + 0.01*torch.randn(len(pestat_DegreeCentrality))).unsqueeze(dim=1)
        #print(data.pestat_DegreeCentrality)
